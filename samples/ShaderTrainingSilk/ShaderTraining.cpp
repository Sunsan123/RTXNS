...
static const char* g_windowTitle = "RTX Neural Shading Example: Silk Shader Training (Ground Truth | Training | Loss )";
...
public:
    ...
    bool Init()
    {
        ...
        std::filesystem::path appShaderPath = app::GetDirectoryWithExecutable() / "shaders/ShaderTrainingSilk" / app::GetShaderTypeName(GetDevice()->getGraphicsAPI());
        ...
        // Initialize direct pass
        {
            m_directPass.constantBuffer = GetDevice()->createBuffer(nvrhi::utils::CreateStaticConstantBufferDesc(sizeof(DirectConstantBufferEntry), "DirectConstantBuffer")
                                                                        .setInitialState(nvrhi::ResourceStates::ConstantBuffer)
                                                                        .setKeepInitialState(true));
            m_directPass.vertexShader = m_shaderFactory->CreateShader("app/renderSilk", "main_vs", nullptr, nvrhi::ShaderType::Vertex);
            m_directPixelShaderAnisotropic = m_shaderFactory->CreateShader("app/renderSilk", "main_ps", nullptr, nvrhi::ShaderType::Pixel);
            m_directPixelShaderOriginal = m_shaderFactory->CreateShader("app/renderSilkOriginal", "main_ps", nullptr, nvrhi::ShaderType::Pixel);
    ...
    void Animate(float seconds) override
    {
        ...
        if (defaultPbrRenderTimeUs != 0)
        {
            ...
            m_extraStatus = std::format(" - Silk - {:3d}us, Neural - {:3d}us, Training - {:3d}us, Optimization - {:3d}us", defaultPbrRenderTimeUs, neuralRenderTimeUs,
        ...
        if (!m_userInterfaceParameters->fileName.empty())
        {
            if (m_userInterfaceParameters->load)
            {
                ...
                if (m_neuralNetwork->InitialiseFromFile(m_userInterfaceParameters->fileName))
                {
                    ...
                    {
                        log::error("Loaded network architecture does not match Original or Anisotropic ShaderTrainingSilk modes.");
                        m_userInterfaceParameters->useAnisotropy = m_activeUseAnisotropy;
                    }
                }
            }
    ...
    void Render(nvrhi::IFramebuffer* framebuffer) override
    {
        ...
        DirectConstantBufferEntry directModelConstant{ {},
                                                       ...
                                                       clampedAnisotropy,
                                                       useAnisotropy ? 1u : 0u,
                                                       std::clamp(m_userInterfaceParameters->specularShift, 0.f, 1.f),
                                                       0.f,
                                                       0.f };
...
private:
    nvrhi::IFramebuffer* GetDepthFramebuffer(nvrhi::IFramebuffer* framebuffer)
    {
            ...
            depthDesc->sampleQuality != fbinfo.sampleQuality)
        {
            ...
            newDepthDesc.debugName = "ShaderTrainingSilkDepth";
...
public:
    ...
    void buildUI() override
    {
        ...
        {
            ImGui::Text("Silk/PBR Render : %d us (%.3f ms)", m_userInterfaceParameters->defaultPbrRenderTimeUs,
            ...
            if (m_userInterfaceParameters->defaultPbrRenderTimeUs > 0 && m_userInterfaceParameters->neuralRenderTimeUs > 0)
            {
                ...
                ImGui::Text("Neural vs Silk/PBR Delta : %+d us", neuralDeltaUs);
                ImGui::Text("Silk/PBR / Neural Speedup : %.2fx", pbrToNeuralSpeedup);
                ImGui::Text("Neural / Silk/PBR Cost : %.2fx", neuralRatio);
            }
        ...
        if (ImGui::Checkbox("Anisotropic Silk", &m_userInterfaceParameters->useAnisotropy))
        {
        ...
        if (m_userInterfaceParameters->useAnisotropy)
        {
            ...
            ImGui::SliderFloat("Specular Shift", &m_userInterfaceParameters->specularShift, 0.f, 1.f);
        }
