...
#define ORIGINAL_INPUT_FEATURES 8
#define ANISOTROPIC_INPUT_FEATURES 8
...
#define COMPONENT_WEIGHTS float4(2.f, 2.f, 2.f, 8.f)
...
struct DirectConstantBufferEntry
{
    ...
    float roughness = 0;
    float metallic = 0;
    float anisotropy = 0;      // Reused as PreintegratedSkin opacity to keep the sample framework compact.
    uint32_t useAnisotropy = 0;
    float specularShift = 0;   // Reused as subsurface color scale.
    float pad1 = 0;
    uint32_t onlyNeuralDebug = 0;
};
